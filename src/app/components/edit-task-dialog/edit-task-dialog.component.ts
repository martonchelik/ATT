...
import {
  DATETIME_FORMAT, MAT_DATETIME_FORMAT,
...
@Component({
  ...
  providers: [
    {
      provide: [NgxMatDateAdapter, MatDatepickerModule, MatNativeDateModule],
    ...
    if (this.form.valid) {
      const task: Task = {
        ...
        createdAt: this.data.isEdit
          ...
          : moment().format('L'),
        dueDate: new Date(this.form.controls.dueDate.value.toISOString()).toLocaleDateString('en-US',{
          year: 'numeric',
          month: '2-digit',
          day: '2-digit',
        }),
